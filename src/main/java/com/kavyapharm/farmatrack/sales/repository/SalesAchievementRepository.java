...
public interface SalesAchievementRepository extends JpaRepository<SalesAchievement, Long> {
        ...
        @Query("SELECT COALESCE(SUM(sa.achievedUnits), 0) FROM SalesAchievement sa " +
                        "WHERE sa.mrId = :mrId " +
                        "AND (:productId IS NULL OR sa.productId = :productId) " +
                        "AND sa.periodMonth = :month AND sa.periodYear = :year")
        Integer sumAchievedUnitsByMrAndProduct(@Param("mrId") Long mrId, @Param("productId") Long productId,
                        @Param("month") Integer month, @Param("year") Integer year);

        @Query("SELECT COALESCE(SUM(sa.achievedUnits), 0) FROM SalesAchievement sa WHERE sa.mrId = :mrId AND sa.periodMonth = :month AND sa.periodYear = :year")
        Integer sumAchievedUnitsByMr(@Param("mrId") Long mrId, @Param("month") Integer month,
