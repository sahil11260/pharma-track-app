...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // âœ… Add Product form submission
  ...
  // âœ… Populate product dropdown (now showing price)
  function populateProductSelect() {
    productSelect.innerHTML = receivedStock
      .map(
        (p) =>
          `<option value="${p.id}">
            ${p.name} â€” â‚¹${p.price}/unit (Available: ${p.available})
          </option>`
      )
      .join("");
    updateAvailableInfo();
  ...
  // âœ… Update available info on product/qty change
  ...
  function updateAvailableInfo() {
    ...
    if (product) {
      ...
      availableInfo.textContent = `Available: ${product.available} units â€” After allocation: ${after >= 0 ? after : "--- (insufficient)"
        } | Price: â‚¹${product.price}/unit`;
  ...
  // âœ… Allocation Role Toggle Logic
  ...
  // âœ… Render received stock summary content
            ...
            <div class="card h-100 shadow-sm border-2 ${bgClass}">
              <div class="card-body p-3">
                ...
                <div>
                  ...
                  <div class="text-muted small">${p.category} | â‚¹${p.price}/unit</div>
                </div>
              </div>
            </div>
          </div>
    ...
    summaryTableBody.innerHTML = displayStock
      .map(p => {
        ...
        <tr>
          ...
          <td class="text-end">â‚¹${p.price}</td>
  ...
  // âœ… Global functions for edit/delete/page change
